...
typedef struct chararr{ char str[512];wchar_t wstr[512] ;} chararr;
typedef struct wchararr{ wchar_t wstr[512];} wchararr;
...
inline chararr** typemap = (chararr**)( dtmem + (uintptr_t)0x227A0 ); 
typedef struct syscallinfo { chararr* syscallname; uint16_t argcount; uint16_t probeid; uint16_t fill[2]; } syscallinfo;
inline syscallinfo* syscallinfos = (syscallinfo* )( dtmem + (uintptr_t)0x23FA0 ); 																		
int tickcount,syscallidx,syscallparamidx; 
string ArgTypeMap[ string, uint16_t ] ;
uint16_t argCount[string];
inline unsigned int maxTicks = 2360; 
typedef union STR{ char str[512];wchar_t wstr[256];} STR;
typedef struct WSTR{ char str[512]; } WSTR;
uint16_t started;
...
:::tick-100/  ++tickcount < maxTicks /
{
	...
	if( syscallinfos[ syscallidx ].probeid == 0xffff ){ syscallidx = syscallidx + 1; } 
...
{	
	...
	marg1=&xlate< STR >( *xlate<typeAndPtr*>(1) ).str[0] ; marg2=&xlate< STR >( *xlate<typeAndPtr*>(2) ).str[0] ; marg3=&xlate< STR >( *xlate<typeAndPtr*>(3) ).str[0] ; marg4=&xlate< STR >( *xlate<typeAndPtr*>(4) ).str[0] ;
	marg5=&xlate< STR >( *xlate<typeAndPtr*>(5) ).str[0] ; marg6=&xlate< STR >( *xlate<typeAndPtr*>(6) ).str[0] ; marg7=&xlate< STR >( *xlate<typeAndPtr*>(7) ).str[0] ; marg8=&xlate< STR >( *xlate<typeAndPtr*>(8) ).str[0] ;	 	 
